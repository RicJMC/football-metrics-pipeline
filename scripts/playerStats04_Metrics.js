...
const METRICAS = {
  ...
  goalkeeper: {
    attributes: ['gk_psxg_zScore', 'gk_psnpxg_per_shot_on_target_against_zScore','gk_psxg_net_per90_zScore','gk_passes_zScore', 'gk_crosses_stopped_pct_zScore', 'gk_def_actions_outside_pen_area_per90_zScore', 'gk_passes_pct_launched_zScore'],
    name: 'gK'
  },
  passing: {
...
function processarArquivo(filename) {
  // Ler os dados do arquivo JSON
  const jsonData = fs.readFileSync(filename);
  const data = JSON.parse(jsonData);

// Nome do arquivo de log
const logFile = 'errorLog04_Metrics.txt';
...
for (const metrica in METRICAS) {
 ...
 const outputFilename = filename.replace('.json', '04_Metrics.json');
 fs.writeFileSync(outputFilename, JSON.stringify(data, null, 2));

 console.log(`As métricas foram adicionadas e normalizadas com sucesso no arquivo "${outputFilename}"!`);
}

// Lista de arquivos a serem processados
const filesToProcess = ['../jsonfiles/playerStats03_ZScores_0.json', '../jsonfiles/playerStats03_ZScores_1.json', '../jsonfiles/playerStats03_ZScores_2.json', '../jsonfiles/playerStats03_ZScores_3.json'];
