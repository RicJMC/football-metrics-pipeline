...
(async () => {
  ...
  await page.goto('https://fbref.com/en/comps/32/gca/Primeira-Liga-Stats');
...
const clickButton = async (page) => {
  await page.waitForSelector('#stats_gca_per_match_toggle');
  await page.evaluate(() => {
    document.querySelector('#stats_gca_per_match_toggle').click();
  });
...
const extractAndSaveData = async (page) => {
  ...
  fs.writeFileSync('gca.json', JSON.stringify(data, null, 2));
  
  // Converte os dados para CSV e grava em arquivo
  const csv = parse(data);
  fs.writeFileSync('gca.csv', csv);
};

// Função para extrair os nomes das colunas
const extractColumnNames = async (page) => {
  return await page.evaluate(() => {
    const columnElements = document.querySelectorAll('#stats_gca th:not(.over_header)');
...
const extractData = async (page) => {
  return await page.evaluate(() => {
      const tableRows = document.querySelectorAll('#stats_gca tr');
      const rowData = [];
