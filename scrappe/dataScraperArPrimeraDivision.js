...
(async () => {
  ...
  const league = 'Primera-Division'; // Liga da Serie A do Brasil
  const competitionId = '21'; // ID da competição da Serie A do Brasil
  const categories = [
    'standard',
    'shooting',
    'passing',
...
const extractCategory = async (browser, url, category, season, league, competitionId) => {
  ...
  try {
    await page.waitForSelector(`#stats_${category}_per_match_toggle`, { timeout: 50000 });
...
//   const league = 'Serie-A'; // Liga da Serie A do Brasil
//   const competitionId = '24'; // ID da competição da Serie A do Brasil
//   const categories = [
//     'shooting',
//     'passing',
//     'gca',
...
//   for (const season of seasons) {
//     for (const category of categories) {
//       const url = `https://fbref.com/en/comps/${competitionId}/${season}/${category}/${season}-${league}-Stats`;
//       const extractionPromise = extractCategory(browser, url, category, season, league, competitionId);
//       extractionPromises.push(extractionPromise);
//     }
//   }
...
// const saveData = async (data, category, season, league, competitionId) => {
//   console.log(`Salvando dados da categoria ${category} da ${league} na temporada ${season}...`);
//   const directoryPath = `./data/${league}/${season}/${competitionId}/${season}-${league}-Stats/${category}/`;
